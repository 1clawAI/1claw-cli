...
billingCommand
  ...
  .action(async (opts) => {
    try {
      ...
      const pct = Math.round((sub.requests_used / sub.requests_limit) * 100);
      const bar = progressBar(pct);

      printKeyValue([
        ["Plan", chalk.bold(sub.plan)],
        ["Status", sub.status === "active" ? chalk.green("Active") : chalk.yellow(sub.status)],
        ["Period ends", sub.current_period_end ? new Date(sub.current_period_end).toLocaleDateString() : "—"],
...
billingCommand
  ...
  .action(async (opts) => {
    try {
      requireToken();
      const balance = await api<CreditBalance>("/billing/credits/balance");
      ...
      printKeyValue([
        ["Balance", chalk.bold(`$${(balance.balance_cents / 100).toFixed(2)}`)],
        [
    ...
    try {
      ...
      printTable(
        txns.map((t) => ({
          ...
          amount:
            t.amount_cents >= 0
              ? chalk.green(`+$${(t.amount_cents / 100).toFixed(2)}`)
              : chalk.red(`-$${(Math.abs(t.amount_cents) / 100).toFixed(2)}`),
