...
import { printKeyValue, printSuccess, printInfo, printWarning } from "../output.js";
...
mfaCommand
  ...
  .action(async () => {
    try {
      ...
      console.log();
      console.log(
        chalk.yellow.bold("  Save these recovery codes in a safe place:"),
      );
      console.log(
        chalk.yellow("  Each code can only be used once."),
      );
      console.log();
      ...
      const { method } = await inquirer.prompt([
        {
          ...
          choices: [
            { name: "TOTP code from authenticator app", value: "totp" },
