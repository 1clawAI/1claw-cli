...
import { printTable, printKeyValue, printSuccess, printInfo, printJson } from "../output.js";
...
vaultCommand
  ...
  .action(async (name, opts) => {
    try {
      ...
      printSuccess(`Vault created: ${chalk.bold(vault.name)} (${vault.id})`);
    } catch (err) {
      handleError(err);
    }
  });
...
vaultCommand
  .command("link <id>")
  .description("Link current directory to a vault (used as default for other commands)")
  .action(async (id) => {
    try {
      requireToken();
      const vault = await api<Vault>(`/vaults/${id}`);
